...
class TaskDispatcher:
    ...
    def register_task_handler(self, dispatcher: Any):
        for handler_func, handling_type_list in TaskDispatcher._get_task_hanlders(type(dispatcher)):
        ...
        if not isinstance(task, TerminateDispatcherLoop):
            ...
            if task_type in self.task_handlers:
                for handler in self.task_handlers[task_type]:
                    ...
                    handler_job = manager.create_child_job(job, handler, task, dispatcher=self)
