...
def test_task_dipatcher_dispatches_tasks_with_redis(task_dispatcher_with_redis: TaskDispatcher):
    dummy_handler = DummyTaskHandler()
    task_dispatcher_with_redis.register_task_handler(dummy_handler)

    all([task_dispatcher_with_redis.post_task(t) for t in [DummyTaskOne(), DummyTaskTwo(), DummyTaskThree()]])

    waiting.wait(lambda: all(mock.called for mock in dummy_handler.mocks), sleep_seconds=0.1, timeout_seconds=30)
