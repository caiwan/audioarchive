...
def test_job_system_single_task(job_manager, job_count):
    ...
    job_fns = [[Mock(side_effect=lambda *a, **w: log_fn(*a, **w)), None] for _ in range(job_count)]
...
def test_job_system_subtasks(job_manager, job_count):
    ...
    wait(lambda: all(mock.called for mock, _j in job_fns), sleep_seconds=0.1, timeout_seconds=1)
